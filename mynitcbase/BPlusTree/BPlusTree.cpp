    ...
    while(StaticBuffer::getStaticBlockType(block) == IND_INTERNAL) {  //use StaticBuffer::getStaticBlockType()
        ...
        InternalEntry intEntry;
         ...
         if (op == NE || op == LT || op == LE) {
            ...
            */
            int numEntries = intHead.numEntries;
            int i;
            for (i = 0; i < numEntries; i++) {
                internalBlk.getEntry(&intEntry, i);
                int cmpVal = compareAttrs(intEntry.attrVal, attrVal, attrCatEntry.attrType);

                if ((op == EQ || op == GE) && cmpVal >= 0) {
                    break;
                } else if (op == GT && cmpVal > 0) {
                    break;
                }
            }

            if (i < numEntries) {
                // (an entry satisfying the condition is found)

                // move to the left child of that entry
                block = intEntry.lChild;

            } else {
                // (no entry in the block satisfies the condition)
